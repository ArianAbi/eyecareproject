...
export default function GuaranteeDialog({
  ...
  async function save() {
    ...
    try {
      ...
      toast.add({
        type: "error",
        title: "نام گارانتی ذخیره نشد؛ دوباره تلاش کنید",
      });
    } finally {
      setSaving(false);
      ...
      <DialogTrigger
        render={
          <Button
            type="button"
            variant="outline"
            disabled={disabled || !cartItemId}
          />
        }
      >
        <span className="max-w-40 truncate">
          {clientName || "درج نام مشتری"}
        </span>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            نام مشتری برای گارانتی این قلم سفارش ذخیره می‌شود. این فیلد اختیاری
            است.
          </DialogDescription>
        </DialogHeader>
        <form
