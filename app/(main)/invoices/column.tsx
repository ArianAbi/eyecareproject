...
export const InvoicesColumn: ColumnDef<InvoiceActionType>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            return <div className="text-center">{InvoicePaymentTypeFarsi(row.original.paymentType)}</div>
        }
    },
    {
        accessorKey: "status",
    ...
    },
    {
        ...
        cell: ({ row }) => {
            if (!row.original.dueDate) return <div className="text-center italic text-muted-foreground">---</div>
            return <div className="text-center">{format(row.original.dueDate, "yyyy/MM/dd")}</div>
        }
    },
    {
