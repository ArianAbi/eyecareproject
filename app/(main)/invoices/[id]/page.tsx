...
export default async function InvoiceDetailPage({ params }: {
    ...
    return <div className="p-3 space-y-4 max-w-2xl">
        ...
        {invoice.data.status === 'PAID' && !invoice.data.creditAppliedAt && <p role="status">?????? ?????? ??? ???? ?????? ?????? ?? ???? ??? ?????? ?? ?????? ???. ?? ?? ???? ??????? ????? ????? ?????? ?? ????? ?? ?? ???????? ???? ??????.</p>}
        {(invoice.data.status === 'PENDING' || (invoice.data.status === 'PAID' && !invoice.data.creditAppliedAt)) && invoice.data.paymentType === 'CASH' &&
            <InvoiceControls 
            id={id} 
            largePayBtn
            />
