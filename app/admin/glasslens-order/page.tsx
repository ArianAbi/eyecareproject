...
export default async function AdminGlasslensOrderPage({
  searchParams,
}: {
  ...
  const catalog = user
    ? await Promise.all([GetProductsAction(), GetProductCategorys(), GetTags()])
    : null;
  return (
    <div className="space-y-4">
      ...
      <section className="space-y-3 rounded-lg border border-dashed p-3">
        <div className="max-w-sm">
          <AdminUserSearchFilter
            key={params.userId ?? "empty"}
            initialValue={
              user
                ? JSON.stringify({ value: user.id, label: user.username })
                : undefined
            }
            paramKey="userId"
          />
        ...
        {user && (
          <>
            ...
            <p className="text-sm text-muted-foreground">
              ...
              سفارش برای همین کاربر اعمال می‌شوند.
            </p>
          </>
        )}
      </section>
      {user && catalog ? (
        ...
        <div className="rounded-lg border border-dashed p-8 text-center text-muted-foreground">
          {userId
            ? "کاربر یافت نشد؛ کاربر دیگری انتخاب کنید."
            : "برای ثبت سفارش، ابتدا یک کاربر انتخاب کنید."}
        </div>
      )}
    </div>
  );
