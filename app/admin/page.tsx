...
export default async function SummaryPage({ searchParams }: { searchParams: Promise<{ day?: string, dailyPage?: string, page?: string, tab?: string }> }) {
    ...
    return <div className="space-y-5">
        ...
        <TodaysOrdersSection page={params.dailyPage} />
        ...
        <Tabs defaultValue="orders" paramKey="tab"><TabsList><TabsTrigger value="orders">سفارش‌ها</TabsTrigger><TabsTrigger value="financial">مالی</TabsTrigger><TabsTrigger value="tickets">تیکت‌ها</TabsTrigger></TabsList>
            ...
            <TabsContent value="tickets" className="space-y-4 pt-4">
                <p>وضعیت فعلی همه تیکت‌ها</p>
                {data.tickets.map(row => <Link className="block rounded-lg border p-4" key={row.status} href={`/admin/tickets?status=${row.status}`}>
                    {row.status === 'OPEN' ? 'تیکت‌های باز' : 'تیکت‌های بسته'}
                    : {row._count}</Link>)}{!data.tickets.length && <p>تیکتی ثبت نشده است.</p>}
                <Link href="/admin/tickets" className="underline">مدیریت تیکت‌ها</Link>
            </TabsContent>
        </Tabs>
    </div>
}
