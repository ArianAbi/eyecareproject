...
const messageSchema = z.string().trim().min(1, "متن پیام را وارد کنید").max(5000)
const ticketSchema = z.object({ subject: z.string().trim().min(3).max(150), message: messageSchema })
...
async function listTickets(filters: Filters, userId?: string) {
    const status: 'OPEN' | 'CLOSED' | undefined = filters.status === 'OPEN' || filters.status === 'CLOSED' ? filters.status : undefined
    const where = { userId, status }
    return prisma.$transaction(async tx => ({
        tickets: await tx.ticket.findMany({ where, ...PaginationObjectDB(filters.page), orderBy: { updatedAt: 'desc' },
            include: { user: { select: { username: true } }, _count: { select: { messages: true } },
                messages: { take: 1, orderBy: { createdAt: 'desc' }, select: { fromAdmin: true } } } }),
...
export async function GetSingleTicketAction(id: string) {
    const user = await requireUser()
    return prisma.ticket.findFirst({ where: { id, userId: user.id }, include: {
...
export async function CreateTicketAction(input: { subject: string, message: string }) {
    const user = await requireUser()
    const value = ticketSchema.parse(input)
    const data = await prisma.$transaction(async tx => {
        const ticket = await tx.ticket.create({ data: { subject: value.subject, userId: user.id,
...
async function reply(id: string, message: string, actorId: string, admin: boolean) {
    const text = messageSchema.parse(message)
    await prisma.$transaction(async tx => {
        // Lock/update the parent first: closing and replying serialize on this row.
        const updated = await tx.ticket.updateMany({ where: { id, status: 'OPEN', ...(!admin ? { userId: actorId } : {}) },
            data: { updatedAt: new Date() } })
        if (!updated.count) throw new Error("تیکت بسته شده یا در دسترس نیست")
        await tx.ticketMessage.create({ data: { ticketId: id, authorId: actorId, fromAdmin: admin, message: text } })
...
async function close(id: string, actorId: string, admin: boolean) {
    await prisma.$transaction(async tx => {
        const updated = await tx.ticket.updateMany({ where: { id, status: 'OPEN', ...(!admin ? { userId: actorId } : {}) }, data: { status: 'CLOSED' } })
        if (!updated.count) throw new Error("تیکت بسته شده یا در دسترس نیست")
