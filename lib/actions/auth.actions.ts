...
export async function CreateUserAction(
    ...
    if (!await allowOperation("signup", await requestIdentity(), 5, 3600000)) return { success: false, error: "Too many attempts. Please try later." };
    ...
    try {
        ...
        const user = await prisma.$transaction(async (tx) => {
            ...
            return { username: user.username, number: user.number };
        });

        await BALE_SendMessage(`${emojis.user} حساب جدید باز شد
