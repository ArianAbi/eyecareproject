...
export async function CreateInvoiceAction(input: {
  ...
  BALE_SendMessage(`${emojis.recipt} درخواست افزایش موجودی
                    کاربر : ${user.username}
...
export async function PayInvoiceAction(invoiceId: string) {
  ...
  if (
    ...
    throw new Error("درخواست نامعتبر است یا امکان انجام این عملیات وجود ندارد");
  // Preserve the authority so concurrent clicks cannot overwrite a payable session.
  if (invoice.zarinpalAuthority)
    return {
      success: true,
      redirectUrl: paymentUrl(invoice.zarinpalAuthority),
    };
  ...
  const { authority } = await zarinpalRequestPayment({
    ...
    callbackUrl: new URL("/invoices/verify", appUrl).toString(),
  });
  const data = await prisma.$transaction(async (tx) => {
    const result = await tx.invoice.updateMany({
      where: { id: invoice.id, status: "PENDING", zarinpalAuthority: null },
      data: { zarinpalAuthority: authority },
    });
    if (result.count)
      await writeAudit(tx, user.id, "PAYMENT_STARTED", "Invoice", invoice.id);
    return tx.invoice.findUniqueOrThrow({ where: { id: invoice.id } });
  });
  if (data.status !== "PENDING" || !data.zarinpalAuthority)
    throw new Error("درخواست نامعتبر است یا امکان انجام این عملیات وجود ندارد");
...
export async function VerifyInvoicePaymentAction(
  ...
  if (!authority || authority.length > 200)
    throw new Error("درخواست نامعتبر است یا امکان انجام این عملیات وجود ندارد");
  const invoice = await prisma.invoice.findUnique({
    where: { zarinpalAuthority: authority, userId: user.id },
  });
  if (!invoice || invoice.paymentType !== "CASH")
    throw new Error("درخواست نامعتبر است یا امکان انجام این عملیات وجود ندارد");
  if (invoice.status === "PAID")
    return { success: true, invoiceId: invoice.id };
  if (status !== "OK" || invoice.status !== "PENDING")
  ...
  await prisma.$transaction(async (tx) => {
    const updated = await tx.invoice.updateMany({
      ...
      data: {
        status: "PAID",
        paidAt: new Date(),
        zarinpalRefId: String(verified.refId),
      },
    });
